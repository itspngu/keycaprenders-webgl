...
namespace KeyboardRender {
    ...
    // DOM
    var appWrapper: HTMLElement;
    var threeContainer: HTMLElement;
    var optionWrapper: HTMLElement;
    var pickerContainer: HTMLElement;
    var clickGuard: HTMLElement;
    ...
    // EXTERNAL MAGIC
    export function getColor(opt: string): string {
        if(setOpts[opt]) return setOpts[opt];
        return undefined;
    }
    ...
    export function setColor(opt: string, cString: string): boolean {
        if(setOpts[opt]) {
            let color = setControlColor(cString);
            if(color != undefined){
    ...
    // Actual Work
    function clickOptionPicker(id, evt): void {
        let loc = evt.srcElement.getBoundingClientRect();
        pickerActive = true;
    ...
    function blurOptionInput(tar): void {
        //@ts-ignore
        let val = document.getElementById(`input-${tar}`).value;
        let initialValue = document.getElementById(`input-${tar}`).getAttribute('value');
        let color = setControlColor(val);
        if(color != undefined){
    ...
    function clickPickerColor(key): void {
        let tar = pickerContainer.getAttribute('data-target');
    ...
    async function loadStuff(): Promise<any>{
        console.log('Loading...');
        await Promise.all(loadPkg.map((loadTar)=>{
            return loadUp(loadTar);
        }));
    }

    function loadUp(tar): Promise<any> {
    ...
    function setupOptions(): void {
        inputOptions.forEach((opt)=>{
            if(opt.type == 'colorInput') colorInputOptionMaker(opt);
        });
        for(let key in gmkColors){
            pickerOptionMaker(key);
        }

    }

    function pickerOptionMaker(key): void {
    ...
    function colorInputOptionMaker(opt): void {
        let initCol = setControlColor(opt.defaultInput);
    ...
    function setControlColor(col): string {
        col = col.toLowerCase();
        if(col == undefined || col == null || col == '') return '';
        if(col.substring(0,1) == '#') return hexToRGB(col);
        if(gmkColors[col]) return gmkColors[col];
    }
    ...
    function hexToRGB(hex): string {
        ...
        let sPar = `rgb(${parseInt(result[1], 16)},${parseInt(result[2],16)},${parseInt(result[3], 16)})`;
        //console.log(sPar);
        return sPar;
    }
	...
	function applyKeycapMaterial(obj): void {
		...
		else if (obj.children[0].material.name.indexOf('mod_background') > -1) obj.children[0].material = materials.modBackground;
		else if (obj.children[0].material.name.indexOf('mod_legends') > -1) obj.children[0].material = materials.modLegends;
		else if (obj.children[0].material.name.indexOf('accent_background') > -1) obj.children[0].material = materials.accentBackground;
		else if (obj.children[0].material.name.indexOf('accent_legends') > -1) obj.children[0].material = materials.accentLegends;
		
		if (obj.children[1].material.name.indexOf('alpha_background') > -1) obj.children[1].material = materials.alphaBackground;
		else if (obj.children[1].material.name.indexOf('alpha_legends') > -1) obj.children[1].material = materials.alphaLegends;
		else if (obj.children[1].material.name.indexOf('mod_background') > -1) obj.children[1].material = materials.modBackground;
		else if (obj.children[1].material.name.indexOf('mod_legends') > -1) obj.children[1].material = materials.modLegends;
		else if (obj.children[1].material.name.indexOf('accent_background') > -1) obj.children[1].material = materials.accentBackground;
		else if (obj.children[1].material.name.indexOf('accent_legends') > -1) obj.children[1].material = materials.accentLegends;
		
		obj.receiveShadow = true;
		obj.castShadow = true;
	}

    function setKeyCap(key): void {
        if(boardMap.alphaBackground.indexOf(key.name) > -1) key.material = materials.alphaBackground;
        if(boardMap.modBackground.indexOf(key.name) > -1) key.material = materials.modBackground;
        if(boardMap.accentBackground.indexOf(key.name) > -1) key.material = materials.accentBackground;
        key.castShadow = true;
        key.receiveShadow = true;
    }

    function setLegend(legend): void {
        if(boardMap.alphaLegends.indexOf(legend.name) > -1) legend.material = materials.alphaLegends;
        if(boardMap.modLegends.indexOf(legend.name) > -1) legend.material = materials.modLegends;
        if(boardMap.accentLegends.indexOf(legend.name) > -1) legend.material = materials.accentLegends;
    }
    ...
    function getMetalTexture(): string {
